...
function tpack_nota_univ(df::DataFrame, type::AbstractString; nota::Symbol=:NT_GER)
    base_layers = data(df) * (linear())
    base_fig = (; resolution=(1600, 1200))
    base_legend = (; position=:bottom, titleposition=:left, framevisible=true, padding=5)
    if type == "t"
...
function tpack_nota_curso_univ(df::DataFrame, type::AbstractString, curso::Integer; nota::Symbol=:NT_GER)
    df_curso = filter(row -> row.CO_GRUPO == curso, df)
    base_layers = data(df_curso) * (linear())
    base_fig = (; resolution=(1600, 1200))
    base_legend = (; position=:bottom, titleposition=:left, framevisible=true, padding=5)
...
save(joinpath(pwd(), "figures", "densidade_enade_univ.png"), density_enade(df_univ))

save(joinpath(pwd(), "figures", "violin_enade.png"), violin_enade(df))
save(joinpath(pwd(), "figures", "violin_enade_univ.png"), violin_enade_univ(df_univ))
...
save(joinpath(pwd(), "figures", "scatter_tpack_t_univ.png"), tpack_nota_univ(df_univ,"t"))
save(joinpath(pwd(), "figures", "scatter_tpack_c_univ.png"), tpack_nota_univ(df_univ,"c"))
save(joinpath(pwd(), "figures", "scatter_tpack_p_univ.png"), tpack_nota_univ(df_univ,"p"))
...
map(x -> save(joinpath(pwd(), "figures", "scatter_tpack_t_curso_$(x)_univ.png"), tpack_nota_curso_univ(df_univ, "t", x)), [1, 2, 12, 2001, 4004])
map(x -> save(joinpath(pwd(), "figures", "scatter_tpack_c_curso_$(x)_univ.png"), tpack_nota_curso_univ(df_univ, "c", x)), [1, 2, 12, 2001, 4004])
map(x -> save(joinpath(pwd(), "figures", "scatter_tpack_p_curso_$(x)_univ.png"), tpack_nota_curso_univ(df_univ, "p", x)), [1, 2, 12, 2001, 4004])
