...
parameters {
  ...
  matrix[K, J1] Z1;                     // group-level non-centered slopes (group 1)
  vector<lower=0>[J2] tau2;             // group-level SD slopes (group 2)
  matrix[K, J2] Z2;                     // group-level non-centered slopes (group 2)
}
transformed parameters {
  matrix[K, J1] betaj1;                   // group-level slopes (group 1)
  for (j in 1:J1) betaj1[, j] = Z1[, j] * tau1[j];
  matrix[K, J2] betaj2;                   // group-level slopes (group 2)
...
model {
  ...
  // likelihood
  y ~ normal(alpha + (X * beta) + (X * betaj1 * tau1) + (X * betaj2 * tau2) , sigma);
}
